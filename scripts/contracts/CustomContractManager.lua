...
function CustomContractManager:getActiveContractsForCurrentFarm()
  ...
  for _, contract in pairs(contractManager.contracts) do
    ...
    if (contract.status == CustomContract.STATUS.ACCEPTED or contract.status == CustomContract.STATUS.CANCELLED or contract.status == CustomContract.STATUS.COMPLETED_AWAITING_INVOICE)
